...
function importExactPrices() {
    ...
    appleModels.forEach(model => {
        ...
        const result = updateOrAddPhone(phones, 'Apple', model, usedPrices, newPrices);
    ...
    samsungModels.forEach(model => {
        ...
        const result = updateOrAddPhone(phones, 'Samsung', model, usedPrices, newPrices);
        if (result === 'updated') updated++;
        else if (result === 'added') added++;
    });
    ...
    console.log('✅ Exact price import completed');
    console.log(`   Updated: ${updated}, Added: ${added}, Total: ${phones.length}`);

    return { updated, added, total: phones.length };
}

function updateOrAddPhone(phones, brand, model, usedPrices, newPrices) {
    const storages = [...new Set([...Object.keys(usedPrices), ...Object.keys(newPrices)])];

    // Find existing phone with multiple fallback strategies
    const existingIndex = phones.findIndex(p => {
        ...
        if (p.brand.toLowerCase() === brand.toLowerCase() &&
            p.model.toLowerCase() === model.toLowerCase()) return true;
        // Fuzzy match
...
console.log('✅ Exact Price Import Utility loaded');
